...
def normalize(X): # creating standard variables here (u-x)/sigma
    if isinstance(X, pd.DataFrame):              #Making sure that the input is a dataframe
        for c in X.columns:                      #Iterating through columns
            if is_numeric_dtype(X[c]):           #Filtering the numeric columns
                u = np.mean(X[c])                #Storing mean of the column
                s = np.std(X[c])                 #Storing std of the column
                X[c] = (X[c] - u) / s            #Normalizing the column
        return
...
def minimize(
    ...
    #while np.linalg.norm(loss_gradient(X, y, B, lmbda)) > precision:
    for i in range(max_iter):
        ...
        #B=B-(eta)*loss_gradient(X,y,B,lmbda)
...
class LogisticRegression621: # MODIFY THIS ONE
    ...
    def predict(self, X):
        ...
        """
        arr_1=self.predict_proba(X)
        arr_2=[1 if p>0.5 else 0 for p in arr_1]
...
class RidgeRegression621: # MODIFY THIS ONE
    ...
    def fit(self, X, y):
        ...
        B_rid=minimize(X, y,
                          ...
                          self.max_iter,
                          addB0=False)
        self.B=np.vstack([[np.mean(y)],B_rid])
