...
def load_docs(docs_dirname:str) -> Sequence[Sequence]:
    ...
    for doc in docs:
        ...
        lists_wordlists.append(words(get_text(doc)))
        
    return lists_wordlists
...
def vocab(neg:Sequence[Sequence], pos:Sequence[Sequence]) -> dict:
    ...
    set_allwords=set()
    wordslists=neg + pos
    for lis in wordslists:
        for word in lis:
            set_allwords.add(word)
            
    set_allwords=sorted(set_allwords)
...
def vectorize(V:dict, docwords:Sequence) -> np.ndarray:
    ...
    for word in docwords:
            if word in V_new.keys():
                V_new[word]+=1
            
            else:
                V_new['__unknown__']+=1'''
    final_array=np.zeros((len(V.keys(),)))
    ...
    #arr1=np.ndarray(list(V_new.values()))           
    return final_array
...
class NaiveBayes621:
    ...
    def fit(self, X:np.ndarray, y:np.ndarray) -> None:
        ...
        rows=np.shape(X)[0]
        y_res=np.reshape(y,(rows,1))
        M=np.hstack((X,y_res))                
        
        p_c=np.sum(y[y==1])/len(y)
        self.log_pc=np.log(p_c)
        
        
        M_c=M[M[:,-1]==1]
        M_c=M_c[:,:-1]
        w_c_sum=M_c.sum(axis=0)+1
        total_c_sum=np.sum(w_c_sum)+1
        pwc=w_c_sum/total_c_sum
        self.log_pwc=np.log(pwc)
        
        p_nc=1 - p_c
        self.log_pnc=np.log(p_nc)
        
        M_nc=M[M[:,-1]==0]
        M_nc=M_nc[:,:-1]
        w_nc_sum=M_nc.sum(axis=0)+1
        total_nc_sum=np.sum(w_nc_sum)
        pwnc=w_nc_sum/total_nc_sum
        self.log_pwnc=np.log(pwnc)
    ...
    def predict(self, X:np.ndarray) -> np.ndarray:
        ...
        # YOUR CODE HERE
        MX=X
        y_c=np.dot(MX,self.log_pwc) 
        y_c=y_c + self.log_pc
        
        y_nc=np.dot(MX,self.log_pwnc)
        y_nc=y_nc + self.log_pnc
...
def kfold_CV(model, X:np.ndarray, y:np.ndarray, k=4) -> np.ndarray:
        ...
        kf = KFold(n_splits=4,random_state=999,shuffle=True )
        accuracies=[]
